...
/**
 * Обновление текстов правовых документов и добавление user-agreement + cookies.
 */
class m250101_000010_update_legal_pages extends Migration
{
    public function safeUp()
    {
        ...
        foreach ($pages as $slug => $data) {
            $existing = $this->db->createCommand("SELECT id FROM {{%page}} WHERE slug = :s", [':s' => $slug])->queryScalar();
            if ($existing) {
                $this->update('{{%page}}', [
                    'title' => $data['title'],
                    'content' => $data['content'],
                    'meta_description' => $data['meta'] ?? null,
                    'is_active' => 1,
                    'updated_at' => $now,
                ], ['slug' => $slug]);
            } else {
        ...
        // Удалим неиспользуемую псевдо-страницу tariffs-info (заменили на /tariffs)
        $this->delete('{{%page}}', ['slug' => 'tariffs-info']);
    }

    public function safeDown()
    {
        // Откат не предусмотрен (текстовые данные, не разрушающее изменение)
    }
    ...
    public static function pages(): array
    {
        return [
            'terms' => [
                'title' => 'Правила пользования сервисом «Вольный Ход»',
                'meta' => 'Правила использования сервиса каршеринга Вольный Ход — требования к водителю, аренда, ответственность',
                'content' => self::terms(),
            ],
            'privacy' => [
                'title' => 'Политика обработки персональных данных',
                'meta' => 'Политика конфиденциальности и обработки персональных данных в сервисе Вольный Ход',
                'content' => self::privacy(),
            ],
            'user-agreement' => [
                'title' => 'Пользовательское соглашение',
                'meta' => 'Пользовательское соглашение — публичная оферта сервиса каршеринга Вольный Ход',
                'content' => self::userAgreement(),
            ],
            'cookies' => [
                'title' => 'Политика использования файлов cookie',
                'meta' => 'Какие cookie использует сайт Вольный Ход и как ими управлять',
...
<p>Срок ответа на обращение — не более 10 рабочих дней.</p>
HTML;
    }

    private static function userAgreement(): string
    {
        return <<<'HTML'
<blockquote>
...
Телефон: <strong>+7 (800) 555-35-35</strong></p>
HTML;
    }
...
<p>Вопросы по использованию cookies: <a href="mailto:privacy@volnyhod.ru">privacy@volnyhod.ru</a>.</p>
HTML;
    }
}
